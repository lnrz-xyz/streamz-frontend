...
export default function Spotify() {
  ...
  useEffect(() => {
    if (isSuccess) {
      redirect("/onboarding/accounts?x_connected=true")
    }
  }, [isSuccess])
  ...
  useEffect(() => {
    if (error) {
      redirect(
        `/onboarding/accounts?error_type=x_connection_error&error=${error.message}`
      )
    }
  }, [error])
  ...
  useEffect(() => {
    if (xError) {
      redirect(
        `/onboarding/accounts?error_type=x_redirect_error&error=${xError}`
      )
    }
  })
